...
function ImageSlider({ images, title, onOpen }: { images: string[]; title: string; onOpen?: (images: string[], idx: number) => void }) {
  ...
  return (
    <div className="slider">
      <img
        ...
        src={images[idx]}
        alt={`${title} ${idx+1}`}
        onClick={() => onOpen && onOpen(images, idx)}
      />
      {images.length > 1 && (
        <>
          <button className="slider-btn slider-prev" onClick={e => { e.stopPropagation(); setIdx((idx - 1 + images.length) % images.length) }}>‹</button>
          <button className="slider-btn slider-next" onClick={e => { e.stopPropagation(); setIdx((idx + 1) % images.length) }}>›</button>
          <div className="slider-dots">
            {images.map((_, i) => (
              <span key={i} className={`slider-dot ${i === idx ? 'active' : ''}`} onClick={e => { e.stopPropagation(); setIdx(i) }} />
            ))}
          </div>
        </>
      )}
    ...
    <div className="lightbox" onClick={onClose}>
      ...
      {images.length > 1 && (
        <>
          ...
          <button className="lightbox-btn lightbox-next" onClick={e => { e.stopPropagation(); setIdx((idx + 1) % images.length) }}>›</button>
          ...
          <div className="projects-grid">
            {visibleProjects.map(pr => (
              <div key={pr.id} className="project-card">
                <ImageSlider images={pr.images || (pr.img ? [pr.img] : [])} title={pr.title} onOpen={(imgs, i) => setLightbox({ images: imgs, idx: i })} />
