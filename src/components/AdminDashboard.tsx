        ...
        {page === 'projects' && (
          <div>
            ...
            {(data.projects||[]).map((pr: any, i: number) => (
              <div key={pr.id} style={s.listItem}>
                <div style={{ width:36, height:36, borderRadius:8, background:'rgba(0,212,255,0.08)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:600, color:'#00D4FF', flexShrink:0 }}>{i+1}</div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{pr.title}</div>
                  <div style={{ fontSize:11, color:'#5A6278', marginTop:2 }}>{pr.tag} · {(pr.chips||[]).slice(0,3).join(', ')}</div>
                </div>
                <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                  <button style={{...s.btn,...s.btnGhost, padding:'5px 10px', fontSize:11}} onClick={() => setModal({ type:'project', idx:i, item:{ ...pr, chips:(pr.chips||[]).join(', ') } })}>Edit</button>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
          ...
          <div style={{ background:'#0F1420', border:'0.5px solid rgba(255,255,255,0.13)', borderRadius:14, padding:28, width:'100%', maxWidth:520 }}>
            ...
            {modal.type === 'project' && (
              <>
                <div style={s.row2}>
                  <div><label style={s.label}>Tag Label</label><input style={s.input} value={modal.item.tag} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,tag:e.target.value}}))} /></div>
                  <div><label style={s.label}>Tag Color</label>
                    <select style={{...s.input}} value={modal.item.tagClass} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,tagClass:e.target.value}}))}>
                ...
                <div style={s.row1}><div><label style={s.label}>Title</label><input style={s.input} value={modal.item.title} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,title:e.target.value}}))} /></div></div>
                <div style={s.row1}><div><label style={s.label}>Description</label><textarea style={{...s.input,minHeight:80,resize:'vertical'}} value={modal.item.desc} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,desc:e.target.value}}))} /></div></div>
                <div style={s.row1}><div><label style={s.label}>Chips (comma separated)</label><input style={s.input} value={modal.item.chips} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,chips:e.target.value}}))} /></div></div>
                <div style={s.row1}><div><label style={s.label}>Image path or URL</label><input style={s.input} value={modal.item.img} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,img:e.target.value}}))} /></div></div>
              </>
            )}

            {modal.type === 'skill' && (
              <>
                <div style={s.row1}><div><label style={s.label}>Group Name</label><input style={s.input} value={modal.item.group} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,group:e.target.value}}))} /></div></div>
                <div style={s.row1}><div><label style={s.label}>Skills (comma separated)</label><input style={s.input} value={modal.item.tags} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,tags:e.target.value}}))} /></div></div>
              </>
            )}
            ...
            {modal.type === 'cert' && (
              <>
                <div style={s.row2}>
                  <div><label style={s.label}>Icon (emoji)</label><input style={s.input} value={modal.item.icon} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,icon:e.target.value}}))} maxLength={4} /></div>
                  <div><label style={s.label}>Name</label><input style={s.input} value={modal.item.name} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,name:e.target.value}}))} /></div>
                </div>
                <div style={s.row1}><div><label style={s.label}>Issuer</label><input style={s.input} value={modal.item.issuer} onChange={e=>setModal((m:any)=>({...m,item:{...m.item,issuer:e.target.value}}))} /></div></div>
              </>
            )}
            ...
            <div style={{ display:'flex', justifyContent:'flex-end', gap:10, marginTop:20 }}>
              ...
              <button style={{...s.btn,...s.btnPrimary}} onClick={() => {
                ...
                if (modal.type === 'project') {
                  const p = { ...item, chips: item.chips.split(',').map((c:string)=>c.trim()).filter(Boolean), id: item.id || Date.now() }
                  setData((d:any) => {
